 ...
 */
  ...
  async requestNewUser(ctx, name, emailID, phoneNumber, aadharNumber) {
    ...
    if (ctx.clientIdentity.getMSPID() != "usersMSP") {
      return "Raising a request for new user is restricted only to Users. Check if you are connected to peer0.users.property-registration-network.com";
    }
    ...
    try {
      ...
      return "Registration request already exists for this user.";
    }
 ...
 */
  ...
  async viewUser(ctx, name, aadharNumber) {
    ...
    try {
      ...
      return "This user is not approved or doesn't exist in the Registered network, Please make sure user exists and approved by the registrar";
    }
  }
...
*/
  ...
  async propertyRegistrationRequest(ctx, propertyId, owner, price, status, name, aadharNumber) {
    ...
    //to chekc if only users peer are used to interact with this function
    if (ctx.clientIdentity.getMSPID() != "usersMSP") {
      return "Only user is authorised to raise property registration request. You are not a user. Please make sure you're connected peer0.users.org.property-registration-network.com";
    }
    ...
    try {
      ...
      return "Request to register this property on the network already exist, please wait until registrar approves the property registration request";
    }
  ...
  async viewProperty(ctx, propertyId) {
    ...
    try {
      ...
      return "This property is either not approved or doesn't exist in the network. Please raise a property registration request and wait until a Registrar approves the property";
    }
  }
 ...
 */
  ...
  async updateProperty(ctx, propertyId, name, aadharNumber, status) {
    //to chekc if only users peer are used to interact with this function
    if (ctx.clientIdentity.getMSPID() != "usersMSP") {
      return "Only user is authorised to raise update property request. You are not a user. Please make sure you're connected peer0.users.org.property-registration-network.com";
    }
    ...
    try {
      ...
      if (propertyDetails.owner == approvedUserKey) {
        ...
        if (status == "registered" || status == "onSale") {
          ...
          return "Status passed to the function is neither registered nor onSale. Please note that the status is case sensitive. Please re-enter status value again.";
        }
      }

      else {
        return "This user doesn't this property";
      }
    }

    catch (err) {
      return "Property either doesn't exist or not approved by the registrar to be on the network";
    }
  }
 ...
 */
  ...
  async purchaseProperty(ctx, propertyId, name, aadharNumber) {
    //to chekc if only users peer are used to interact with this function
    if (ctx.clientIdentity.getMSPID() != "usersMSP") {
      return "Only user is authorised to raise property purchase request. You are not a user. Please make sure you're connected peer0.users.org.property-registration-network.com";
    }
      ...
      if (propertyDetails.status == "onSale") {
        ...
        try {
          ...
          if (approvedUserData.upGradCoins >= propertyDetails.price) {
            ...
            return "You do not have enough upGradCoins to buy this property, Please recharge your account";
          }
        }
